...
// get selected image
$selected_image = basename(rawurldecode($_GET["image"]));
...
if(!$selected_image) {
	...
	$resize_opt = $_GET["resize"];

	// check if user specified resize
  if(!in_array($resize_opt, $config["resize_options"])) {
   if(in_array($_COOKIE["resize"], $config["resize_options"])) {
     $resize_opt = $_COOKIE["resize"];
   } else {
